import { ArrowRight, BookOpen, Users, Target, TrendingUp, Clock } from 'lucide-react';
...
export function InsightsGuidanceSection() {
  const { ref, isVisible } = useScrollAnimation();
  const [, setLocation] = useLocation();

  const handleInsightClick = (insightId: string) => {
    setLocation(`/insights/${insightId}`);
  };
    ...
    <section ref={ref} className="py-20 bg-gradient-to-br from-gray-900 to-gray-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {insightsOptions.map((insight, index) => {
              ...
              >
                <div className="bg-white rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 h-full">
                  ...
                  <div className="relative h-56 overflow-hidden">
                    ...
                    {/* Icon overlay */}
                    <div className={`absolute top-4 right-4 ${insight.color} w-12 h-12 rounded-full flex items-center justify-center shadow-lg`}>
